...
export default function StoriesLibraryPage() {
  ...
  const completed = PRESET_STORIES.filter(story => profile.completedStories.includes(story.id)).length;
  const next = PRESET_STORIES.find(story => !profile.completedStories.includes(story.id));
  return <div className="learning-page">
    ...
    <section aria-labelledby="adventure-map-heading">
      <div className="section-heading"><div><p className="eyebrow">Bab 01 · Kenalan dengan uang</p><h2 id="adventure-map-heading">Peta petualanganmu</h2></div><span>{completed} / {PRESET_STORIES.length} selesai</span></div>
      <progress className="learning-progress mb-8" value={completed} max={Math.max(1, PRESET_STORIES.length)} aria-label="Cerita selesai" />
      {!ready ? <p role="status" className="py-10">Membuka peta petualangan...</p> : PRESET_STORIES.length === 0 ? <p role="status">Cerita baru sedang disiapkan. Coba buat cerita sendiri dulu, ya!</p> : <ol className="adventure-path">{PRESET_STORIES.map((story, index) => {
        ...
        return <li key={story.id} className={`adventure-stop ${done ? 'stop-done' : unlocked ? 'stop-current' : 'stop-locked'}`}>
          ...
          <article className="story-stop-card">
            <div className={`story-scene ${index % 2 ? 'scene-forest' : 'scene-space'}`} aria-hidden="true"><span className="scene-orbit" /><span className="scene-world" /><Image src="/mascot/dino.png" alt="" width={130} height={130} /><span className="scene-label">{index % 2 ? 'Hutan Ajaib' : 'Luar Angkasa'}</span></div>
            <div className="story-stop-copy"><p className="eyebrow">Misi 0{index + 1} · {story.quiz.length} pertanyaan</p><h3>{story.title}</h3><p>{story.description}</p>
      ...
      })}<li className="path-finish"><Flag size={22} aria-hidden="true" /><span>{completed === PRESET_STORIES.length ? 'Semua misi selesai. Hebat, petualang!' : 'Selangkah demi selangkah, kamu pasti bisa.'}</span></li></ol>}
    </section>
  </div>;
}
