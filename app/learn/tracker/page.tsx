...
export default function TrackerPage() {
    ...
    const handleAddGoal = (goalData: { title: string; targetAmount: number; deadline?: string }) => {
        ...
        if (goals.length === 0) setPrimaryGoalId(newId);
        ...
        setGoals(goals.map(g => {
            if (g.id === goalId) {
                ...
                return {
                    ...
                    status: updated >= g.targetAmount ? 'completed' : 'in_progress'
                };
            }
            return g;
        }));
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="bg-surface p-6 sm:p-8 rounded-3xl shadow-sm border border-border space-y-4">
                            <h3 className="font-heading font-bold text-xl text-brand-primary">Catatan Uang Hari Ini</h3>
                            {transactions.length === 0 ? (
                                ...
                                <div className="divide-y divide-border max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                                    {transactions.map((t) => {
                                        ...
                                        return (
                                            <div key={t.id} className="py-3 flex justify-between items-center group hover:bg-surface-soft px-2 rounded-xl transition-colors">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-xl ${t.type === 'income' ? 'bg-brand-secondary/30 text-brand-primary' : 'bg-destructive/10 text-destructive'}`}>
                                                        {t.type === 'income' ? '+' : '-'}
                                                    </div>
                                                    <div>
                                                ...
                                                <span className={`font-bold text-sm px-3 py-1 rounded-lg ${t.type === 'income' ? 'text-brand-primary bg-brand-secondary/20' : 'text-destructive bg-destructive/10'}`}>
